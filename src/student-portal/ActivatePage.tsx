...
import { activate, checkAuthToken } from '@/lib/api';
...
export default function ActivatePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    checkAuthToken(token, 'activation')
      .then((res) => {
        if (cancelled) return;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      const res = await activate(token, password, confirm);
  ...
  return (
    <div className="academy-page academy-auth-page">
      <StudentAuthNav active="none" />
      <main className="academy-auth-main">
        <section className="academy-auth-card academy-auth-card-large" aria-labelledby="activate-title">
          <div className="academy-auth-card-glow" aria-hidden="true" />
          <div className="academy-auth-heading">
            <span>{t.auth.loginKicker}</span>
            <h1 id="activate-title">{t.portal.activateTitle}</h1>
          ...
          <form className="academy-auth-form" onSubmit={onSubmit}>
            ...
            <button className="academy-btn academy-btn-primary academy-auth-submit" type="submit" disabled={pending}>
              {pending ? t.portal.submitting : t.portal.activateSubmit}
            </button>
          </form>
        </section>
      </main>
